...
class TestMisc(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.hashfile_digests = {
            "SHA256": "4f075ae76b480bb0200dab01cd304f4045e04cd2b73e88b89549e5ac1627f222",
    ...
    def test_call_external_command_pass(self):
        ...
        misc.call_external_command(
            command, with_log_files=True, prefix="test", log_dir=log_dir
        )
    ...
    def test_call_external_command_detached(self):
        ...
        misc.call_external_command_detached(
            command, with_log_files=True, prefix="test_det"
        )
        time.sleep(0.1)
...
class TestFilesystem(unittest.TestCase):
    ...
    def test_crete_folder_parent_non_existing(self):
        ...
        target_folder = os.path.join(
            self.rootdir, "parent-non-existing", "target-non-existing"
        )
        self.assertTrue(
            filesystem.create_folder(target_folder),
...
class TestTransferAgent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.agent = transfer.TransferAgent(
            src_path=self.rootdir, dest_path=self.destdir
        )
...
class TestSymlinkAgent(unittest.TestCase):
    ...
    def test_symlink_file_make_dir(self):
        ...
        target = os.path.join(
            self.targetdir, "these", "folders", "should", "be", "created"
        )
        self.assertTrue(transfer.SymlinkAgent(src, target).transfer())
    ...
    def test_symlink_file_unlink_fail(self):
        ...
        with mock.patch.object(
            transfer.os, "unlink", side_effect=OSError("Mocked error")
        ):
    ...
    def test_symlink_folder_rmtree_fail(self):
        ...
        with mock.patch.object(
            transfer.shutil, "rmtree", side_effect=OSError("Mocked error")
        ):
            with self.assertRaises(transfer.SymlinkError):
                transfer.SymlinkAgent(src, target).transfer()
    ...
    def test_symlink_folder_symlink_error(self):
        ...
        target = os.path.join(self.targetdir, os.path.basename(src))
        with mock.patch.object(
            transfer.os, "symlink", side_effect=OSError("Mocked error")
        ):
            with self.assertRaises(transfer.SymlinkError):
                transfer.SymlinkAgent(src, target).transfer()
    ...
    def test_symlink_folder_unexpected(self):
        ...
        target = self.targetdir
        with mock.patch.object(
            transfer.os.path, "exists", side_effect=Exception("Mocked error")
        ):
...
class TestRsyncAgent(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        (fh, cls.testfile) = tempfile.mkstemp(prefix="test_taca_transfer_file")
        ...
        def _write_digest(rootdir, fhandle, fpath):
            fhandle.write(
                f"{misc.hashfile(fpath)}  {os.path.relpath(fpath, rootdir)}\n"
            )
        ...
        with open(cls.digestfile, "w") as digesth:
            map(
                lambda x: [
                    _write_digest(cls.rootdir, digesth, os.path.join(x[0], y))
                    for y in [
                        z for z in x[2] if os.path.join(x[0], z) != cls.digestfile
                    ]
                ],
                os.walk(cls.rootdir),
            )
    ...
    def setUp(self):
        ...
        self.agent = transfer.RsyncAgent(
            self.rootdir, dest_path=self.destdir, validate=False
        )
    ...
    def test_rsync_validate_transfer(self):
        ...
        self.assertFalse(
            self.agent.validate_transfer(),
            "validation with an invalid digestfile should return false",
        )
    ...
    def test_rsync_agent_propagate_error(self):
        ...
        with mock.patch.object(
            ...
            side_effect=subprocess.CalledProcessError(
                cmd="mocked subprocess", returncode=-1
            ),
        ):
    ...
    def test_rsync_agent_file(self):
        ...
        self.assertTrue(
            self.validate_files(
                self.agent.src_path,
                os.path.join(self.destdir, os.path.basename(self.agent.src_path)),
            ),
    ...
    def test_rsync_agent_symlink(self):
        ...
        self.assertTrue(
            self.agent.transfer(), "transfer a folder containing a symlink failed"
        )
        self.assertEqual(
            misc.hashfile(self.testfile, hasher="sha1"),
            misc.hashfile(
                os.path.join(self.destdir, "folder0", "link1"), hasher="sha1"
            ),
    ...
    def validate_files(self, src, dst):
        return (
            ...
            and misc.hashfile(src) == misc.hashfile(dst)
        )
...
class TestConfig(unittest.TestCase):
    def test_load_yaml_config(self):
        ...
        expexted_config_data = {
            "statusdb": {"url": "url", "username": "username", "password": "pwd"},
            "log": {"file": "data/taca.log"},
        }
...
class TestBioinfoTab(unittest.TestCase):
    ...
    def test_collect_runs(self, mock_update_statusdb):
        ...
        calls = [
            mock.call("data/test_data/190201_A00621_0032_BHHFCFDSXX"),
            mock.call("data/test_data/nosync/190201_A00621_0032_BHHFCFDSXY"),
        ]
    ...
    def test_get_ss_projects(self):
        ...
        expected_info = defaultdict(
            bioinfo_tab.Tree,
            {
                "HHFCFDSXX": defaultdict(
                    bioinfo_tab.Tree,
                    {
                        "1": defaultdict(
                            bioinfo_tab.Tree,
                            {
                                "P10000_1001": defaultdict(
                                    bioinfo_tab.Tree,
    ...
    def test_update_statusdb(self, mock_couch):
        ...
        mock_couch.StatusdbSession.assert_called_with(
            {
                "url": "url",
                "username": "username",
                "password": "pwd",
    ...
    def test_parse_sample_sheet_is_miseq(self):
        ...
        parsed_data = bioinfo_tab.parse_samplesheet(
            sample_sheet, "run_dir", is_miseq=True
        )
        self.assertEqual(expected_data, parsed_data)
...
class TestStatusdb(unittest.TestCase):
    ...
    def test_get_entry(self, mock_couch):
        ...
        couch_config = {"user": "username", "url": "some_url", "password": "some_pwd"}

...
def process_minion_qc_run(minion_run, sequencing_ongoing=False, nanoseq_ongoing=False):
    ...
    if (
        ...
        and not os.path.isdir(minion_run.nanoseq_dir)
    ):
        logger.info(
        ...
        if os.path.isfile(minion_run.nanoseq_sample_sheet):
            if nanoseq_ongoing:
                logger.warning(
                    "Nanoseq already started, will not attempt to start for {}".format(
                        minion_run.run_dir
                    )
                )
            elif sequencing_ongoing:
                logger.warning(
                    "Sequencing ongoing, will not attempt to start Nanoseq for {}".format(
                        minion_run.run_dir
                    )
                )
            else:
            ...
            logger.warning(
                "Samplesheet not found for run {}. Operator notified. Skipping.".format(
                    minion_run.run_dir
                )
            )
            ...
            email_message = (
                "There was an issue locating the samplesheet for run {}.".format(
                    minion_run.run_dir
                )
            )
        ...
        logger.info(
            "Nanoseq has started for run {} but is not yet done. Skipping.".format(
                minion_run.run_dir
            )
        )
        ...
        if nanoseq_successful:
            if not os.path.isdir(minion_run.anglerfish_dir):
                logger.info(
                    "Nanoseq done for run {}. Attempting to start Anglerfish.".format(
                        minion_run.run_id
                    )
                )
        ...
        logger.info(
            "Run {} not finished sequencing yet. Skipping.".format(minion_run.run_id)
        )
...
def process_minion_qc_runs(run, nanoseq_sample_sheet, anglerfish_sample_sheet):
    ...
    if run:
        if is_date(os.path.basename(run).split("_")[0]):
            minion_run = MinIONqc(
                os.path.abspath(run), nanoseq_sample_sheet, anglerfish_sample_sheet
            )
        ...
        for run_dir in runs_to_process:
            minion_run = MinIONqc(
                run_dir, nanoseq_sample_sheet, anglerfish_sample_sheet
            )
            nanoseq_ongoing = process_minion_qc_run(
                minion_run,
